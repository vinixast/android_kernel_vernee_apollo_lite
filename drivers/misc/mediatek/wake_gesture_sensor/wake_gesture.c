...
static ssize_t wag_show_enable_nodata(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct wag_context *cxt = NULL;
...
/*----------------------------------------------------------------------------*/
static ssize_t wag_show_active(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct wag_context *cxt = NULL;
...
static ssize_t wag_show_delay(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
static ssize_t wag_show_batch(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
static ssize_t wag_show_flush(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
/* cxt->wag_ctl.enable_nodata = ctl->enable_nodata; */
	...
	err = sysfs_create_group(&wag_context_obj->mdev.this_device->kobj, &wag_attribute_group);
	if (err < 0) {
...
static int wag_remove(void)
{
	...
	sysfs_remove_group(&wag_context_obj->idev->dev.kobj, &wag_attribute_group);

	err = misc_deregister(&wag_context_obj->mdev);
	if (err)
